...
module.exports = {
    ...
    run: async (client, message, args, Discord) => {
        ...
        if(!member) return message.reply({
            content: `:x: | يجب عليك منشن الاداري الذي تريد تصفير نقاطه`
        })
        if(member.user.bot) return message.reply({
            content: `:x: | لا تستطيع تصفير نقاط البوتات`
        })
        if(!member.roles.cache.has(data.staff_role)) return message.reply({
            content: `:x: | لا يمكنك تصفير نقاط غير الاداريين`
        })
        ...
        await message.reply({
            content: `:white_check_mark: | تم تصفير نقاط الاداري ${member}`
        })
    }
};
