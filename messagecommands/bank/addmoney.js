...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if (!user) return message.reply({
      content: `:x: | يجب عليك منشن الشخص الذي تريد اضافة فلوس له`
    })
    ...
    await message.reply({
      embeds: [new Discord.EmbedBuilder().setColor("#003d66").setDescription(` | تم إضافة ( ${money} ) للشخصية ${character === "1" ? "الاولى" : "الثانية"} بنجاح`)]
    })
    ...
    log.send({
      embeds: [new Discord.EmbedBuilder().setColor("#003d66").setDescription(`**  - Add Money By | ${message.author} 
