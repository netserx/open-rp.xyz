...
module.exports = {
    ...
    run: async (interaction, client) => {
        ...
        if (interaction.customId.startsWith("addMoney")) {
            ...
            if(db.cash < money) return interaction.reply({
                content: `:x: | رصيدك الحالي أقل من المبلغ الذي تريد إيداعه داخل البنك`,
                ephemeral: true
            })

            let embed = new Discord.EmbedBuilder()
                .setColor("#003d66")
                .setDescription(`**  - سيتم اتمام عملية الايداع هل انت متاكد من ارادة اتمام العملية ؟**`);

            let row = new Discord.ActionRowBuilder().addComponents(
                new Discord.ButtonBuilder()
                    .setCustomId(`confirmm_addMoney_${money}_${check.character}`)
                    .setLabel("إيداع")
                    .setStyle("Success"),

                new Discord.ButtonBuilder()
                    .setCustomId(`cancel_addMoney`)
                    .setLabel("إلغاء")
                    .setStyle("Danger")
            )
